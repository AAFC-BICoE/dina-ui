...
If you would like to ignore the type you can use the no type option:
...
<AutoSuggestTextField<Person>
  ...
  elasticSearchBackend={{
    indexName: "dina_agent_index",
    searchField: "data.attributes.name",
    option: _, noType => noType?.anotherField ?? "" // This part you can now say .name since the type was defined.
  }}
/>
----

With the noType you won't get any type checking, but you will get the full object. This can be useful when dealing with resources from included.
