# UI Common React Component Types
...
| tooltipImageAlt | Accessibility text which should be used when a tooltipImage is provided. Best practice is to use internationalization key so it works in multiple languages. | tooltipImageAlt="tooltip_image_alt"
...
The FilterBuilderField component provides a Formik input for creating complex filter queries. The FilterGroupModel value this input provides can be converted to an FIQL string to be included in the API request.
...
import { fiql } from "../components/filter-builder/fiql";
