...
import { emptyQueryTree, FormikButton, TextField, useAccount } from "..";
import { PcrBatchSelectField } from "../../../dina-ui/components";
...
export interface QueryPage2Props<TData extends KitsuResource> {
  /**
 ...
 * * Saved Searches
 */
export function QueryPage2<TData extends KitsuResource>({
  indexName,
  columns,
  bulkDeleteButtonProps,
  bulkEditPath,
  ...
  const [submittedQueryBuilderTree, setSubmittedQueryBuilderTree] =
    useState<ImmutableTree>(emptyQueryTree());
  ...
  useEffect(() => {
    ...
    // let queryDSL = elasticSearchFormatExport(submittedQueryBuilderTree, queryBuilderConfig);
    ...
    //         },
    //         included: rslt._source?.included?.reduce((includedAccumulator, currentIncluded) => {
    //           if (currentIncluded?.type === 'organism') {
    //             if (!includedAccumulator[currentIncluded?.type]) {
    //               return (includedAccumulator[currentIncluded?.type] = [currentIncluded]), includedAccumulator;
    //             } else {
    //               return includedAccumulator[currentIncluded?.type].push(currentIncluded), includedAccumulator;
    //             }
    //           } else {
    //             return (includedAccumulator[currentIncluded?.type] = currentIncluded), includedAccumulator;
    //           }
  ...
  return (
    <DinaForm key={formKey} initialValues={defaultGroups} onSubmit={onSubmit}>
      ...
      <div
        ...
        role="search"
        aria-label={formatMessage("queryTable")}
      >
        <div className="row">
