...
export function useQueryBuilder({
  ...
  /**
   * By submitting the query tree, you are taking the current version and setting it as the
   * submitted version. This allows the user to keep editing it without it being submitted each
   * time it changes.
   */
  const submitQueryBuilderTree = () => {
    setSubmittedQueryBuilderTree(queryBuilderTree);
    setSessionStorageQueryTree(Utils.getTree(queryBuilderTree));
  };

  /**
   * Reset the query builder tree to the default setup.
   */
  const setDefaultQueryBuilderTree = () => {
  ...
  /**
   * Reset the query builder to an empty tree setup.
   */
  const setEmptyQueryBuilderTree = () => {
    setSubmittedQueryBuilderTree(emptyQueryTree());
    setQueryBuilderTree(emptyQueryTree());
  };

  const updateQueryBuilderTree = (updatedTree) => {
    setQueryBuilderTree(updatedTree);
  };

  const updateSubmittedQueryBuilderTree = (updatedTree) => {
