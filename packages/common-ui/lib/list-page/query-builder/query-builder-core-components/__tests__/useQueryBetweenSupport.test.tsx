import {
  convertBetweenStateToString,
  convertStringToBetweenState,
  isBetweenStateObject,
  isBetweenStateString
} from "../useQueryBetweenSupport";
...
describe("useQueryBetweenSupport hook and utility functions", () => {
  test("isBetweenStateObject", async () => {
    ...
    expect(
      isBetweenStateObject({ low: "1998-05-19", high: "2024-05-19" })
    ).toBe(true);
  ...
  test("isBetweenStateString", async () => {
    // Correct cases
    expect(isBetweenStateString(JSON.stringify({ low: 0, high: 1 }))).toBe(
      true
    );
    expect(
      isBetweenStateString(
        JSON.stringify({ low: "1998-05-19", high: "2024-05-19" })
      )
    ).toBe(true);
  ...
  test("convertStringToBetweenState", async () => {
    ...
    expect(
      convertStringToBetweenState('{"small":"2","big":"0"}')
    ).toMatchObject({
      low: "",
      high: ""
    });
  ...
  test("convertBetweenStateToString", async () => {
    expect(convertBetweenStateToString({ low: "0", high: "1" })).toEqual(
      '{"low":"0","high":"1"}'
    );
    expect(convertBetweenStateToString({ small: "0", big: "1" })).toEqual(
      '{"low":"","high":""}'
    );
  });
});
