...
describe("ClientSideJoiner", () => {
  ...
  it("Joins Metadatas to Persons", async () => {
    const metadatas = getTestMetadatas();
  ...
  it("Throws an error if the bulk fetch fails.", async () => {
    const metadatas = getTestMetadatas();
    ...
    mockBulkGet.mockImplementationOnce(async () => {
      return [new Error("test error 1"), new Error("test error 2")];
    });

    // Should throw the error:
    await expect(clientSideJoiner.join()).rejects.toEqual(
