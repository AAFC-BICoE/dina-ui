...
(function($, window, document, wb) {
  ...
  var componentName = "wb-geomap",
    ...
    init = function(event) {
      ...
      if (event.currentTarget === elm) {
        ...
        overrides = {
          useScaleLine:
            className.indexOf("scaleline") !== -1 ? true : undefined,
          useMousePosition:
            className.indexOf("position") !== -1 ? true : undefined,
          ...
          useGeolocation:
            className.indexOf("geolocation") !== -1 ? true : false,
          ...
          $.ajax({
            ...
            success: function() {
              ...
              if (
                settings &&
                settings.basemap &&
                settings.basemap.type &&
                settings.basemap.type === "xyz"
              ) {
    ...
    Geomap = function(options) {
      ...
      this.mapLayers = [];
      this.layerDiv = $elm
        .find(".wb-geomap-layers")
        .attr("id", "geomap-layers-" + this.id);
      this.legendDiv = $elm
        .find(".wb-geomap-legend")
        .attr("id", "geomap-legend-" + this.id);
      this.mapDiv = $elm
        .find(".wb-geomap-map")
        .attr("id", "geomap-map-" + this.id);
      ...
      this.settings.aspectRatio =
        ...
        this.settings.basemap.mapOptions.aspectRatio !== undefined
          ? this.settings.basemap.mapOptions.aspectRatio
          : 0.8;
    ...
    MapLayer = function(map, options) {
      ...
      this.settings.accessible =
        typeof this.settings.accessible === "undefined"
          ? true
          : this.settings.accessible;
      this.settings.visible =
        typeof this.settings.visible === "undefined"
          ? true
          : this.settings.visible;
      this.settings.zoom =
        typeof this.settings.zoom === "undefined" ? true : this.settings.zoom;

      // Add a section to hold the data table
      if (this.settings.accessible) {
        this.map.layerDiv.append(
          "<div class='panel panel-default'><div class='panel-heading'><div class='panel-title' role='heading'>" +
            this.settings.title +
            "</div></div><div class='panel-body'><div data-layer='" +
            this.id +
            "' class='geomap-table-wrapper' style='display:none;'></div></div></div>"
        );
      }
      ...
      this.observeVisibility(function(vis) {
        var $table = $(
          "div[ data-layer='" + _this.id + "' ].geomap-table-wrapper"
        );
        ...
        if (!vis) {
          ...
          $table
            .parent()
            .append(
              "<div class='layer-msg'><p>" + i18nText.hiddenLayer + "</p></div>"
            )
            .fadeIn();
        } else {
          $table.fadeIn();
    ...
    setRendererDimensions = function(
      ...
      if (!resolution) {
        resolution =
          Math.max(gw / symbolWidth || 0, gh / symbolHeight || 0) || 1;
      }
      map.setView(
        new ol.View({
    ...
    StyleFactory = function() {
      ...
      var RuleStyle = function(feature) {
        ...
        for (var i = 0; i !== len; i += 1) {
          ...
          opacity = rule.init.fillOpacity
            ? rule.init.fillOpacity
            : rule.init.graphicOpacity
            ? rule.init.graphicOpacity
            : 0.5;
          radius = rule.init.pointRadius ? rule.init.pointRadius : 5;
          strokeColor = rule.init.strokeColor
            ? hexToRGB(rule.init.strokeColor, opacity)
            : colors.transparent;
          fillColor = hexToRGB(rule.init.fillColor, opacity);
          graphicName = rule.init.graphicName ? rule.init.graphicName : null;
          externalGraphic = rule.init.externalGraphic
            ? rule.init.externalGraphic
            : null;
          graphicHeight = rule.init.graphicHeight
            ? rule.init.graphicHeight
            : 25;
          graphicWidth = rule.init.graphicWidth ? rule.init.graphicWidth : 25;

          if (
            operators[ruleFilter](feature.attributes[rule.field], rule.value)
          ) {
            switch (featureType) {
      ...
      var SymbolStyle = function() {
        // Set the style elements
        opacity = style.init.fillOpacity
          ? style.init.fillOpacity
          : style.init.graphicOpacity
          ? style.init.graphicOpacity
          : 1.0;
        radius = style.init.pointRadius ? style.init.pointRadius : 5;
        strokeColor = style.init.strokeColor
          ? hexToRGB(style.init.strokeColor, opacity)
          : colors.transparent;
        fillColor = hexToRGB(style.init.fillColor, opacity);
        graphicName = style.init.graphicName ? style.init.graphicName : null;
        externalGraphic = style.init.externalGraphic
          ? style.init.externalGraphic
          : null;
        graphicHeight = style.init.graphicHeight
          ? style.init.graphicHeight
          : 25;
      ...
      var DefaultStyle = function() {
        ...
        fillColor = style.fillColor
          ? hexToRGB(style.fillColor, opacity)
          : colors.transparent;
        strokeColor = style.strokeColor
          ? hexToRGB(style.strokeColor, opacity)
          : colors.transparent;
      ...
      var UniqueStyle = function(feature, featureType) {
        ...
        for (obj in style.init) {
          ...
          opacity = objStyle.fillOpacity
            ? objStyle.fillOpacity
            : objStyle.graphicOpacity
            ? objStyle.graphicOpacity
            : 0.5;
          radius = objStyle.pointRadius ? objStyle.pointRadius : 5;
          strokeColor = objStyle.strokeColor
            ? hexToRGB(objStyle.strokeColor, opacity)
            : colors.transparent;
          fillColor = objStyle.fillColor
            ? hexToRGB(objStyle.fillColor, opacity)
            : null;
    ...
    addChkBox = function(mapLayer, feature) {
      return (
        "<td><label class='wb-inv' for='cb_" +
        feature.getId() +
        "'>" +
        i18nText.labelSelect +
        "</label><input type='checkbox' id='cb_" +
        feature.getId() +
        "' class='geomap-cbx' data-map='" +
        mapLayer.map.id +
        "' data-layer='" +
        feature.layerId +
        "' data-feature='" +
    ...
    addZoomTo = function(mapLayer, feature) {
      return (
        "<td class='text-right'><a href='javascript:;' data-map='" +
        mapLayer.map.id +
        "' data-layer='" +
        feature.layerId +
        "' data-feature='" +
        feature.getId() +
        "' class='btn btn-link geomap-zoomto' alt='" +
        i18nText.zoomFeature +
        "' role='button'><span class='glyphicon glyphicon-zoom-in'></span></a></td>"
      );
    },
    /**
     * Show popup
    ...
    createOLMap = function(geomap) {
      var controls = geomap.settings.useMapControls
          ? ol.control.defaults({
              attributionOptions: {
                collapsible: false
              }
          ...
          : [],
        interactions = geomap.settings.useMapControls
      ...
      map.once("postrender", function() {
        ...
        map.getLayer = function(strSelector) {
          var geometryElm = document.querySelector(
              strSelector + " [data-geometry][data-type]"
            ),
        ...
        map.zoomToExtent = function(layerCoordinates) {
          map
            ...
            .setCenter(
              ol.proj.transform(
                [layerCoordinates[0], layerCoordinates[1]],
                "EPSG:4326",
                "EPSG:3978"
              )
            );
      ...
      geomap.mapDiv.append(
        "<div id='tooltip_" +
          geomap.id +
          "' style='display:none;'><span class='tooltip-txt'></span></div>"
      );
    ...
    AOIWidget = function(geomap) {
      ...
      interaction.on("boxend", function() {
        var extent = interaction
          .getGeometry()
          .transform(projMap, projLatLon)
          .getExtent();
        ...
        geomap.map
          .getView()
          .fit(geomProj.getGeometry().getExtent(), geomap.map.getSize());
        ...
        $("#geomap-aoi-extent-" + geomap.id)
          .val(geomProj.getGeometry().getExtent())
          .trigger("change");
        $("#geomap-aoi-extent-lonlat-" + geomap.id)
          .val(
      ...
      interaction.on("boxstart", function() {
        getLayerById(geomap.map, "locLayer")
          .getSource()
          .clear(true);
      });
      ...
      if (geomap.settings.useAOIOpen) {
        geomap.mapDiv.before(
          "<div class='geomap-aoi panel panel-default'><div id='geomap-aoi-" +
            geomap.id +
            "' class='panel-body'></div></div>"
        );
      } else {
        geomap.mapDiv.before(
          "<details class='geomap-aoi'><summary>" +
            i18nText.aoiTitle +
            "</summary><div id='geomap-aoi-" +
            geomap.id +
            "'></div></details>"
        );
      }
      ...
      aoiDiv.append(
        "<fieldset id='form-aoi-" +
          geomap.id +
          "'>" +
          "<legend tabindex='-1'>" +
          i18nText.aoiInstructions +
          "</legend>" +
          "<div class='row'>" +
          "<div class='col-md-2 form-group'>" +
          "<label for='geomap-aoi-maxy-" +
          geomap.id +
          "' class='wb-inv'>" +
          i18nText.aoiNorth +
          "</label>" +
          ...
          i18nText.aoiNorth.charAt(0) +
          "</span>" +
          "<input type='number' id='geomap-aoi-maxy-" +
          geomap.id +
          "' placeholder='90' class='form-control input-sm' min='-90' max='90' step='0.000001'></input>" +
          "</div>" +
          "</div>" +
          "<div class='col-md-2 form-group'>" +
          "<label for='geomap-aoi-maxx-" +
          geomap.id +
          "' class='wb-inv'>" +
          i18nText.aoiEast +
          "</label>" +
          "<div class='input-group input-group-sm'>" +
          "<span class='input-group-addon'>" +
          i18nText.aoiEast.charAt(0) +
          "</span>" +
          "<input type='number' id='geomap-aoi-maxx-" +
          geomap.id +
          "' placeholder='180' class='form-control input-sm' min='-180' max='180' step='0.000001'></input> " +
          "</div>" +
          "</div>" +
          "<div class='col-md-2 form-group'>" +
          "<label for='geomap-aoi-miny-" +
          geomap.id +
          "' class='wb-inv'>" +
          i18nText.aoiSouth +
          "</label>" +
          "<div class='input-group input-group-sm'>" +
          "<span class='input-group-addon'>" +
          i18nText.aoiSouth.charAt(0) +
          "</span>" +
          "<input type='number' id='geomap-aoi-miny-" +
          geomap.id +
          "' placeholder='-90' class='form-control input-sm' min='-90' max='90' step='0.000001'></input> " +
          "</div>" +
          "</div>" +
          "<div class='col-md-2 form-group'>" +
          "<label for='geomap-aoi-minx-" +
          geomap.id +
          "' class='wb-inv'>" +
          i18nText.aoiWest +
          "</label>" +
          "<div class='input-group input-group-sm'>" +
          "<span class='input-group-addon'>" +
          i18nText.aoiWest.charAt(0) +
          "</span>" +
          "<input type='number' id='geomap-aoi-minx-" +
          geomap.id +
          "' placeholder='-180' class='form-control input-sm' min='-180' max='180' step='0.000001'></input> " +
          "</div>" +
          "</div>" +
          "<div class='col-md-4'>" +
          "<button class='btn btn-default btn-sm' id='geomap-aoi-btn-draw-" +
          geomap.id +
          "'>" +
          i18nText.add +
          "</button> " +
          "<button class='btn btn-default btn-sm' id='geomap-aoi-btn-clear-" +
          geomap.id +
          "'>" +
          i18nText.aoiBtnClear +
          "</button> " +
          "</div>" +
          "</div>" +
          "<input type='hidden' id='geomap-aoi-extent-" +
          geomap.id +
          "'></input>" +
          "<input type='hidden' id='geomap-aoi-extent-lonlat-" +
          geomap.id +
          "'></input>" +
          "</fieldset>"
      );

      $("#geomap-aoi-btn-clear-" + geomap.id).after(
        "<button id='geomap-aoi-toggle-mode-draw-" +
          geomap.id +
          "' href='#' class='btn btn-sm btn-default geomap-geoloc-aoi-btn' title='" +
          i18nText.aoiBtnDraw +
      ...
      $document.on(
        "click",
        "#geomap-aoi-toggle-mode-draw-" + geomap.id,
        function(event) {
          event.preventDefault();

          var drawInteraction = getMapInteraction(
              geomap.map,
              ol.interaction.DragBox
            ),
            selectInteraction = getMapInteraction(
              geomap.map,
      ...
      $document.on("click", "#geomap-aoi-btn-clear-" + geomap.id, function(
        event
      ) {
        event.preventDefault();
        ...
        $("#geomap-aoi-minx-" + geomap.id)
          .val("")
          .parent()
          .removeClass("has-error");
        $("#geomap-aoi-miny-" + geomap.id)
          .val("")
          .parent()
          .removeClass("has-error");
        $("#geomap-aoi-maxx-" + geomap.id)
          .val("")
          .parent()
          .removeClass("has-error");
        $("#geomap-aoi-maxy-" + geomap.id)
          .val("")
          .parent()
          .removeClass("has-error");
      ...
      $document.on("click", "#geomap-aoi-btn-draw-" + geomap.id, function(
        event
      ) {
        event.preventDefault();
    ...
    drawAOI = function(geomap, extent, dontAddFeat) {
      ...
      for (len = dens.length - 1; len !== -1; len -= 1) {
        coords.push([
          dens[len].getCoordinates()[0],
          dens[len].getCoordinates()[1]
        ]);
      }
    ...
    HelpControl = function(geomap) {
      ...
      button.addEventListener(
        ...
        function() {
          ...
          dialog.className =
            "panel panel-default geomap-help-dialog ol-control ";
          ...
          closeButton.innerHTML =
            "&#xd7;<span class='wb-inv'>" + i18nText.dismiss + "</span>";
    ...
    GeocodeControl = function(geomap) {
      var element = document.createElement("div"),
        ...
        mapObj = {
          ...
          "colombie-britannique":
            "<abbr title='Colombie-Britannique'>BC</abbr>",
          ...
          "newfoundland and labrador":
            "<abbr title='Newfoundland and Labrador'>NL</abbr>",
          "terre-neuve-et-labrador":
            "<abbr title='Terre-Neuve-et-Labrador'>NL</abbr>",
          "prince edward island":
            "<abbr title='Prince Edward Island'>PE</abbr>",
          "île-du-prince-édouard":
            "<abbr title='Île-du-prince-Édouard'>PE</abbr>",
          ...
          "northwest territories":
            "<abbr title='Northwest Territories'>NT</abbr>",
          "territoires du nord-ouest":
            "<abbr title='Territoires du Nord-Ouest'>NT</abbr>",
          nunavut: "<abbr title='Nunavut'>NU</abbr>"
        },
        iconObj = {
          "ca.gc.nrcan.geoloc.data.model.Street":
            "<span class='glyphicon glyphicon-road' aria-hidden='true'></span>",
          "ca.gc.nrcan.geoloc.data.model.Intersection":
            "<span class='glyphicon glyphicon-road' aria-hidden='true'></span>",
          "ca.gc.nrcan.geoloc.data.model.Geoname":
            "<span class='glyphicon glyphicon-map-marker' aria-hidden='true'></span>",
          "ca.gc.nrcan.geoloc.data.model.PostalCode":
            "<span class='glyphicon glyphicon-envelope' aria-hidden='true'></span>",
          "ca.gc.nrcan.geoloc.data.model.NTS":
      ...
      element.innerHTML =
        "<label for='wb-geomap-geocode-search-" +
        geomap.id +
        "' class='wb-inv'>" +
        i18nText.geoCoderLabel +
        "</label>" +
        "<input type='text' class='form-control' id='wb-geomap-geocode-search-" +
        geomap.id +
        "' placeholder='" +
        i18nText.geoCoderPlaceholder +
        "' ></input>" +
        "<div class='wb-geomap-geoloc-al-cnt'><ul role='listbox' id='wb-geomap-geoloc-al-" +
        geomap.id +
        "' class='wb-geomap-geoloc-al hide' aria-hidden='true' aria-live='polite'></ul></div>";
      ...
      $("#wb-geomap-geocode-search-" + geomap.id).attr(
        "aria-autocomplete",
        "list"
      );
      $("#wb-geomap-geocode-search-" + geomap.id).attr(
        "aria-owns",
        "wb-geomap-geoloc-al-" + geomap.id
      );
      $("#wb-geomap-geocode-search-" + geomap.id).attr(
      ...
      function panZoomToFeature(bbox, ll) {
        ...
        if (bbox && typeof bbox !== "undefined") {
          ...
          feat = new ol.Feature({
            geometry: new ol.geom.Polygon([coords]).transform(
              projLatLon,
              projMap
            )
          });
          ...
          geomap.map
            .getView()
            .fit(feat.getGeometry().getExtent(), geomap.map.getSize());
        } else if (ll && typeof ll !== "undefined") {
          zoom =
            geomap.map.getView().getZoom() === 0
              ? 12
              : geomap.map.getView().getZoom();
          feat = new ol.Feature({
            geometry: new ol.geom.Point(ll.split(",")).transform(
              projLatLon,
              projMap
            )
          });
      ...
      $document.on(
        "keydown",
        "#wb-geomap-geocode-search-" + geomap.id,
        function(event) {
          // TODO: find another way to do this, maybe setting negative tabindex
          getMapInteraction(geomap.map, ol.interaction.KeyboardPan).setActive(
            false
          );
            ...
            if (data.length > 0) {
              ...
              for (i = 0; i !== len; i += 1) {
                ...
                bnd = item.bbox
                  ...
                  : "";
                ll =
                  item.geometry && item.geometry.type === "Point"
                    ? item.geometry.coordinates[0] +
                      ", " +
                      item.geometry.coordinates[1]
                    : "";
                icon = iconObj[item.type]
                  ? iconObj[item.type]
                  : "<span class='glyphicon glyphicon-map-marker' aria-hidden='true'></span>";
                ...
                options +=
                  ...
                  title +
                  "</span>" +
                  "<span class='al-lbl wb-inv' aria-hidden='true'>" +
                  label +
                  "</span>" +
                  "</a></li>";
              }
      ...
      function keyboardHandlerInput(which, event) {
        ...
        if (!(event.ctrlKey || event.altKey || event.metaKey)) {
          ...
          if (
            ...
            (which === 38 || which === 40) &&
            input.getAttribute("aria-activedescendent") === ""
          ) {
            if (autolistHidden) {
              showOptions(input);
            }
            ...
            input.setAttribute(
              "aria-activedescendent",
              dest.parentNode.getAttribute("id")
            );
      ...
      function clickHandlerAutolist(eventTarget) {
        ...
        panZoomToFeature(
          link.parentNode.getAttribute("data-bbox"),
          link.parentNode.getAttribute("data-lat-lon")
        );
    ...
    GeolocationControl = function(options) {
      ...
      _this.geolocation.on("change:position", function() {
        ...
        _this
          .getMap()
          .getView()
          .fit(extent, _this.getMap().getSize());

        $(button).html(
          "<span style='color:#3399CC;' class='glyphicon glyphicon-screenshot'></span>"
        );
      });
      ...
      _this.geolocation.on("error", function(error) {
        if (error.code === 2) {
          $("#overlay-location-error h2.modal-title").text(
            i18nText.geolocUncapable
          );
      ...
      button.addEventListener(
        ...
        function() {
          $(this).html(
            "<span style='font-size:.9em;' class='glyphicon glyphicon-refresh glyphicon-spin'></span>"
          );
          ...
          if (typeof _this.featuresOverlay === "undefined") {
            ...
            _this.featuresOverlay.getSource().getFeatures().length === 0
          ) {
            _this.featuresOverlay.getSource().addFeatures(createFeatures());
            _this.geolocation.setTracking(true);
          } else {
    ...
    getMapLayerFeature = function(elm) {
      ...
      if (elm.getAttribute("data-layer")) {
        ...
        return [
          geomap.map,
          layer,
          elm.getAttribute("data-feature")
            ? layer.getSource().getFeatureById(elm.getAttribute("data-feature"))
            : null
        ];
      } else {
  ...
  $document.on(wb.resizeEvents, function(event) {
    mapArray.forEach(function(geomap) {
      ...
      if ($myDiv) {
        if (
          event.type === "mediumview" ||
          event.type === "largeview" ||
          event.type === "xlargeview"
        ) {
          $myDiv.css({ width: "60%" });
        } else {
          $myDiv.css({ width: "80%" });
  ...
  $document.on(
    ...
    function(event) {
      var target = event.currentTarget,
        ...
        mouseWheelZoom = getMapInteraction(
          geomap.map,
  ...
  Geomap.prototype.addBasemap = function() {
    ...
    if (hasBasemap) {
      ...
      viewOptions.extent =
        basemap.mapOptions && basemap.mapOptions.maxExtent
          ? basemap.mapOptions.maxExtent.split(",").map(Number)
          : null;
      viewOptions.projection =
        basemap.mapOptions && basemap.mapOptions.projection
          ? basemap.mapOptions.projection
          : "EPSG:3857";
      viewOptions.center =
        _this.settings && _this.settings.center
          ? ol.proj.transform(
              _this.settings.center,
              "EPSG:4326",
              viewOptions.projection
            )
          : basemap.mapOptions && basemap.mapOptions.center
          ? ol.proj.transform(
              basemap.mapOptions.center,
              "EPSG:4326",
              viewOptions.projection
            )
          : basemap.mapOptions && basemap.mapOptions.maxExtent
          ? ol.extent.getCenter(viewOptions.extent)
          : [0, 0];
      viewOptions.zoom =
        _this.settings && _this.settings.zoom
          ? _this.settings.zoom
          : basemap.mapOptions && basemap.mapOptions.zoomLevel
          ? basemap.mapOptions.zoomLevel
          : 2;
  ...
  MapLayer.prototype.addToLegend = function() {
    ...
    if ($fieldset.length === 0) {
      $fieldset = $(
        "<fieldset name='legend'><legend class='wb-inv'>" +
          i18nText.toggleLayer +
          "</legend></fieldset>"
      ).appendTo(legendDiv);
    }
    ...
    if ($ul.length === 0) {
      $ul = $("<ul class='list-unstyled geomap-lgnd'></ul>").appendTo(
        $fieldset
      );
    }
    ...
    $li = $("<li class='checkbox geomap-lgnd-layer'>").append(
      $label,
      "<div id='sb_" + this.id + "'></div>"
    );
    ...
    if (this.settings.options && this.settings.options.legendUrl) {
      $("#sb_" + this.id).append(
        "<img src='" +
          this.settings.options.legendUrl +
          "' alt='" +
          i18nText.geoLgndGrphc +
          "'/>"
      );
  ...
  Geomap.prototype.addTabularData = function() {
    var $table,
      ...
      lenTable,
      thZoom =
        "<th><span class='wb-inv'>" + i18nText.zoomFeature + "</span></th>",
    ...
    for (
      lenTable = this.settings.tables.length - 1;
      lenTable !== -1;
      lenTable -= 1
    ) {
      ...
      $table = $(table).wrap(
        "<div data-layer='" +
          this.settings.tables[lenTable].id +
          "' class='geomap-table-wrapper'></div>"
      );
      ...
      if (
        $table.hasClass("wb-tables") &&
        typeof $table.attr("data-wb-tables") === "undefined"
      ) {
        $table.attr(
          "data-wb-tables",
          '{ "order": [], "columnDefs": [ { "targets": [ 0, ' +
            (thElms.length + 1) +
            ' ], "orderable": false } ] }'
        );
      }
      ...
      visibility =
        this.settings.tables[lenTable].visible === false ? false : true;
      ...
      style =
        typeof featureTable.style === "undefined"
        ...
        if (geomType !== null) {
          if (geomType === "bbox") {
            ...
            for (vertLen = feat.length - 1; vertLen !== -1; vertLen -= 1) {
              vertices +=
                feat[vertLen].getCoordinates()[0] +
                " " +
                feat[vertLen].getCoordinates()[1] +
                ", ";
            }
          ...
          if (featureTable.tooltips) {
            vectorFeature.tooltip = featureTable.tooltipText
              ? attrMap[featureTable.tooltipText]
              : attrMap[Object.keys(attrMap)[0]];
          }

          // Set the table row id
          trElmsInd.setAttribute(
            "id",
            vectorFeature.getId().replace(vectRegex, "_")
          );

          // Add the checkboxes and zoom controls
          $(trElmsInd).html(
            addChkBox(this, vectorFeature) +
              trElmsInd.innerHTML +
              (useMapControls && featureTable.zoom
                ? addZoomTo(this, vectorFeature)
                : "")
          );
  ...
  MapLayer.prototype.populateDataTable = function() {
    ...
    var _this = this,
      ...
      $table = $(
        "<table class='table' aria-label='" +
          _this.settings.title +
          "' id='" +
          _this.id +
          "'><caption>" +
          _this.settings.caption +
          "</caption></table>"
      ),
    ...
    for (key in attributes) {
      if (attributes.hasOwnProperty(key)) {
        attKey = attributes[key].alias
          ? attributes[key].alias
          : attributes[key];
        head += "<th>" + attKey + "</th>";
      }
    }
    ...
    head =
      "<thead><tr>" +
      head +
      (_this.map.settings.useMapControls && _this.settings.zoom
        ? "<th><span class='wb-inv'>" + i18nText.zoomFeature + "</span></th>"
        : "") +
      "</tr></thead>";
    ...
    for (var i = 0; i < features.length; i += 1) {
      ...
      body +=
        _this.map.settings.useMapControls && _this.settings.zoom
          ? addZoomTo(_this.map, features[i])
          : "";
    }

    $table.append(head, "<tbody>" + body + "</tbody>");

    $("div[ data-layer='" + _this.id + "'].geomap-table-wrapper").append(
      $table
    );
  ...
  MapLayer.prototype.createOLLayer = function() {
    ...
    if (_this.settings.type === "wms") {
          ...
          if (source.getState() === "ready") {
            ...
            for (var i = 0, len = features.length; i < len; i += 1) {
              ...
              if (_this.settings.tooltips) {
                feature.tooltip = _this.settings.tooltipText
                  ? atts[_this.settings.tooltipText]
                  : atts[Object.keys(atts)[0]];
              }
            }
      ...
      olSource.once("addfeature", function() {
        olLayer.setStyle(
          styleFactory.createStyleFunction(
            _this.settings.style,
            featureGeometry
          )
        );
        ...
        for (i = 0, len = features.length; i < len; i += 1) {
          ...
          if (
            ...
            geom = new ol.geom.Point([
              feature[geomKey].coordinates[1],
              feature[geomKey].coordinates[0]
            ]);
          ...
          geomProj = geom.transform(
            "EPSG:4326",
            _this.map.map.getView().getProjection()
          );
          ...
          for (var name in layerAttributes) {
            ...
            if (layerAttributes.hasOwnProperty(name)) {
              ...
              if (path) {
                atts[layerAttributes[name].alias] = feature[name]
                  ? feature[name][path]
                  : "";
              } else {
                atts[layerAttributes[name]] = feature[name]
                  ? feature[name]
                  : "";
              }
            }
      ...
      _this.settings.type === "geojson" ||
      _this.settings.type === "esrijson" ||
      _this.settings.type === "topojson"
    ) {
      var layerURL;
      ...
      layerURL = _this.settings.params
        ? _this.settings.url + "?" + $.param(_this.settings.params)
        : _this.settings.url;
    ...
    selectInteraction.on(
      ...
      function(event) {
        ...
        if (
          selected &&
          selected.length > 0 &&
          typeof selected[0].layerTitle !== "undefined"
        ) {
  ...
  Geomap.prototype.createPopup = function() {
    ...
    $popup = $(
      "<div id='popup-" + _this.mapDiv.attr("id") + "' class='ol-popup'></div>"
    );
    $closer = $(
      "<a href='#' title='" +
        i18nText.dismiss +
        "' class='ol-popup-closer' role='button'>&#xd7;<span class='wb-inv'>" +
        i18nText.dismiss +
        "</span></a>"
    );
  ...
  MapLegend.prototype.symbolize = function(mapLayer) {
      ...
      if (ruleLen) {
        for (j = 0; j !== ruleLen; j += 1) {
          ...
          if (filter && !rule.name) {
            if (filter.name) {
              ...
              switch (filter) {
                ...
                case "BETWEEN":
                  title =
                    rule.field + " " + rule.value[0] + " - " + rule.value[1];
                  break;
              }
            }
          ...
          symbolList +=
            ...
            "<div id='" +
            spanId +
            "' class='geomap-legend-symbol'></div>" +
            "<span class='geomap-legend-symbol-text'><small>" +
            title +
            "</small></span>" +
            "</div>" +
            "</li>";
    ...
    $("#sb_" + layerName).html(
      "<ul class='list-unstyled'>" + symbolList + "</ul>"
    );
  ...
  wb.doc.on("click", ".wb-geomap-filter [type=reset]", function() {
    ...
    OLmap.getView().fit(
      mapGetView.calculateExtent(OLmap.getSize()),
      OLmap.getSize()
    );
