!(function(a) {
  function b(b, f) {
    var h = f.Canvas;
    null == c &&
      ((d = h.prototype.getTextInfo),
      (e = h.prototype.addText),
      (c = h.prototype.render)),
                ...
                for (var k in i)
                  if (g.call(i, k)) {
                    ...
                    for (var o, p = 0; (o = m[p]); p++)
                      if (o.active)
                        for (var q, r = 0; (q = o.lines[r]); r++)
                          a.fillText(n[r].text, q[0], q[1]);
                      else m.splice(p--, 1);
        ...
        if (
          ...
          if ("object" != typeof f) {
            var n = a("<div>&nbsp;</div>")
              ...
              .addClass("string" == typeof f ? f : null)
              .appendTo(this.getTextLayer(c));
            (f = {
              lineHeight: n.height(),
          ...
          (i =
            ...
            (l = k[e] = {
              ...
              positions: [],
              lines: [],
              font: { definition: i, color: f.color }
            }),
            m.save(),
            (m.font = i);
          for (
            ...
            (l.width = Math.max(r.width, l.width)),
              ...
              l.lines.push({ text: q, width: r.width, height: f.lineHeight });
          }
          m.restore();
        }
        return l;
      ...
      (h.prototype.addText = function(a, c, d, f, g, h, i, j, k) {
        ...
        (d += l.height / n.length / 2),
          (d =
            "middle" == k
              ? Math.round(d - l.height / 2)
              : "bottom" == k
              ? Math.round(d - l.height)
              : Math.round(d)),
          window.opera && window.opera.version().split(".")[0] < 12 && (d -= 2);
        for (var o, p = 0; (o = m[p]); p++)
          if (o.x == c && o.y == d) return void (o.active = !0);
        (o = { active: !0, lines: [], x: c, y: d }), m.push(o);
        for (var q, p = 0; (q = n[p]); p++)
          "center" == j
            ? o.lines.push([Math.round(c - q.width / 2), d])
            : "right" == j
            ? o.lines.push([Math.round(c - q.width), d])
            : o.lines.push([Math.round(c), d]),
            (d += q.height);
      });
  }
  var c,
  ...
  a.plot.plugins.push({ init: b, options: f, name: "canvas", version: "1.0" });
})(jQuery);
