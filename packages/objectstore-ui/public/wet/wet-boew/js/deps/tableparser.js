    ...
    init = function(event) {
      ...
      function processColgroup(elem, nbvirtualcol) {
        ...
        if (colgroup.col.length === 0) {
          ...
          for (
            i = colgroup.start, iLen = colgroup.start + colgroupspan;
            i !== iLen;
            i += 1
          ) {
            col = {
      ...
      function processRowgroupHeader(colgroupHeaderColEnd) {
        ...
        if (colgroupHeaderColEnd && colgroupHeaderColEnd > 0) {
          ...
          if (
            colgroupFrame.length > 0 &&
            (colgroupFrame[0].start !== 1 ||
        ...
        if (
          ...
          for (i = 0, iLen = groupZero.col.length; i !== iLen; i += 1) {
            ...
            for (j = 0, jLen = tmpStack.length; j !== jLen; j += 1) {
              for (m = gzCol.start, mLen = gzCol.end; m <= mLen; m += 1) {
                ...
                if (
                  (j === 0 ||
                    (j > 0 && cell.uid !== tmpStack[j - 1].cell[m - 1].uid)) &&
                  cell.type === 1
                ) {
          ...
          $.each(colgroupFrame, function() {
            ...
            if (curColgroupFrame.start < currColPos) {
              ...
              for (
                i = 0, iLen = curColgroupFrame.col.length;
                i !== iLen;
                i += 1
              ) {
                ...
                for (j = 0, jLen = tmpStack.length; j !== jLen; j += 1) {
                  for (m = gzCol.start, mLen = gzCol.end; m <= mLen; m += 1) {
                    if (
                      (j === 0 ||
                        (j > 0 &&
                          tmpStack[j].cell[m - 1].uid !==
                            tmpStack[j - 1].cell[m - 1].uid)) &&
            ...
            for (i = 0, iLen = tmpStack.length; i !== iLen; i += 1) {
              ...
              if (
                !tmpStackCell &&
                curColgroupFrame.end > tmpStack[i].cell.length
              ) {
              ...
              if (
                tmpStackCell.colpos + tmpStackCell.width - 1 ===
            ...
            for (
              i = groupLevel - 1, iLen = tmpStack.length;
              i !== iLen;
              i += 1
            ) {
              ...
              for (
                j = curColgroupFrame.start - 1, jLen = curColgroupFrame.end;
                j !== jLen;
                j += 1
              ) {
            ...
            for (
              i = currColgroupStructure.length, iLen = groupLevel - 1;
              i !== iLen;
              i += 1
            ) {
              ...
              if (
                tmpStackCell.uid !==
                  tmpStack[i].cell[curColgroupFrame.end - 1].uid ||
                tmpStackCell.colpos > curColgroupFrame.start ||
                tmpStackCell.colpos + tmpStackCell.width - 1 <
                  curColgroupFrame.end
              ) {
            ...
            for (
              i = groupLevel - (groupLevel >= 2 ? 2 : 1),
                iLen = tmpStack.length;
              i !== iLen;
              i += 1
            ) {
              for (
                j = curColgroupFrame.start;
                j <= curColgroupFrame.end;
                j += 1
              ) {
            ...
            for (i = currColgroupStructure.length - 1; i !== -1; i -= 1) {
              if (currColgroupStructure[i].end <= curColgroupFrame.end) {
                if (
                  currColgroupStructure[i].level < groupLevel &&
                  theadRowStack.length > 0
                ) {
                ...
                if (curColgroupFrame.type === 3 && !summaryAttached) {
                  currColgroupStructure[
                    currColgroupStructure.length - 1
                  ].summary = curColgroupFrame;
            ...
            $.each(curColgroupFrame.col, function() {
              ...
              for (j = groupLevel - 1; j < tmpStack.length; j += 1) {
                for (
                  i = curColgroupFrame.start - 1;
                  i < curColgroupFrame.end;
                  i += 1
                ) {
                  ...
                  if (
                    (colpos >= colStart && colpos <= colEnd) ||
                    (colpos <= colStart && colpos + cellWidth >= colEnd) ||
                    ...
                    if (
                      colHeaderLen === 0 ||
                      (colHeaderLen > 0 &&
                        column.header[colHeaderLen - 1].uid !== cell.uid)
                    ) {
          ...
          $.each(groupZero.virtualColgroup, function() {
            ...
            for (
              i = vGroupHeaderCell.start - 1;
              i < vGroupHeaderCell.end;
              i += 1
            ) {
      ...
      function rowgroupSetup(forceDataGroup) {
        ...
        if (rowgroupHeaderRowStack.length !== 0) {
          ...
          for (
            i = 0, iLen = rowgroupHeaderRowStack.length;
            i !== iLen;
            i += 1
          ) {
        ...
        if (
          rowgroupHeaderRowStack.length === 0 &&
          lstRowGroup.length > 0 &&
          !currentRowGroup.type &&
          colgroupFrame[0] &&
          (colgroupFrame[0].type === 1 ||
            (!colgroupFrame[0].type && colgroupFrame.length > 0)) &&
          !forceDataGroup
        ) {
          currentRowGroup.type = 3;
        } else {
        ...
        if (!currentRowGroup.level) {
          ...
          if (lstRowGroup.length > 0) {
            ...
            if (currentRowGroup.type === 2) {
              // Data Group
              if (
                currentRowGroup.headerlevel.length ===
                previousRowGroup.headerlevel.length
              ) {
                ...
                for (
                  ...
                  currentRowGroup.headerlevel.push(
                    previousRowGroup.headerlevel[i]
                  );
                }
              ...
              for (i = 0; i < previousRowGroup.headerlevel.length; i += 1) {
                if (
                  previousRowGroup.headerlevel[i].level < currentRowGroup.level
                ) {
                  currentRowGroup.headerlevel.push(
                    previousRowGroup.headerlevel[i]
                  );
        ...
        for (i = 0; i < currentRowGroup.headerlevel.length; i += 1) {
          ...
          currentRowGroup.headerlevel[i].rowlevel =
            currentRowGroup.headerlevel[i].level;
        }
      ...
      function processRow(elem) {
        ...
        fnParseSpannedRowCell = function() {
          ...
          while (columnPos <= tableCellWidth) {
            ...
            if (
              ...
              currCell.colpos === columnPos
            ) {
              if (
                currCell.height + currCell.rowpos - currCell.spanHeight !==
                currentRowPos
              ) {
                break;
              }
          ...
          var $this = $(this),
            width =
              $this.attr("colspan") !== undefined
                ? parseInt($this.attr("colspan"), 10)
                : 1,
            height =
              $this.attr("rowspan") !== undefined
                ? parseInt($this.attr("rowspan"), 10)
                : 1,
            headerCell,
            dataCell,
            i;
        ...
        if (lastCellType === "th") {
          ...
          if (row.colgroup.length === 2 && currentRowPos === 1) {
            ...
            if (
              row.colgroup[0].type === 2 &&
              row.colgroup[0].cell.length === 1
            ) {
          ...
          if (row.colgroup.length === 1) {
            if (row.colgroup[0].cell.length > 1) {
              ...
              if (
                currentRowPos !== 1 ||
                row.cell[0].uid === row.cell[row.cell.length - 1].uid
              ) {
          ...
          if (
            ...
            ].cell[0].descCell = row.cell[0];
            row.cell[0].describe.push(
              rowgroupHeaderRowStack[rowgroupHeaderRowStack.length - 1].cell[0]
            );
            if (!groupZero.desccell) {
              groupZero.desccell = [];
            }
          ...
          if (
            colgroupFrame[0] &&
            lastHeadingColPos &&
            colgroupFrame[0].end !== lastHeadingColPos &&
            colgroupFrame[0].end === lastHeadingColPos + 1
          ) {
            ...
            for (i = 0; i < lastHeadingColPos; i += 1) {
              ...
              if (row.cell[i].elem.nodeName.toLowerCase() === "td") {
                if (
                  !row.cell[i].type &&
                  row.cell[i - 1] &&
                  !row.cell[i - 1].descCell &&
                  row.cell[i - 1].type === 1 &&
                  row.cell[i - 1].height === row.cell[i].height
                ) {
                  row.cell[i].type = 5;
              ...
              if (row.cell[i].elem.nodeName.toLowerCase() === "th") {
                ...
                for (j = 0; j < colKeyCell.length; j += 1) {
                  if (
                    !colKeyCell[j].type &&
                    !row.cell[i].keycell &&
                    colKeyCell[j].height === row.cell[i].height
                  ) {
                    colKeyCell[j].type = 4;
          ...
          if (lastHeadingColPos !== 0) {
            lastHeadingColPos =
              colgroupFrame[0].end; /* colgroupFrame must be defined here */
          }
          ...
          for (i = lastHeadingColPos; i < row.cell.length; i += 1) {
            ...
            for (
              j = lastHeadingColPos === 0 ? 0 : 1;
              j < colgroupFrame.length;
              j += 1
            ) {
              // If colgroup, the first are always header colgroup
              if (
                colgroupFrame[j].start <= row.cell[i].colpos &&
                row.cell[i].colpos <= colgroupFrame[j].end
              ) {
            ...
            if (row.cell[i].rowpos < currentRowPos) {
              ...
              if (row.header) {
                for (j = 0; j < row.header.length; j += 1) {
                  if (
                    (row.header[j].rowpos === currentRowPos &&
                      row.cell[i].addrowheaders.length === 0) ||
                    (row.header[j].rowpos === currentRowPos &&
                      row.cell[i].addrowheaders[
                        row.cell[i].addrowheaders.length - 1
                      ].uid !== row.header[j].uid)
                  ) {
          ...
          for (i = 0; i < groupZero.col.length; i += 1) {
            for (
              j = groupZero.col[i].start - 1;
              j < groupZero.col[i].end;
              j += 1
            ) {
              ...
              if (
                !(
                  j > groupZero.col[i].start - 1 &&
                  groupZero.col[i].cell[groupZero.col[i].cell.length - 1]
                    .uid === row.cell[j].uid
                )
              ) {
                if (row.cell[j]) {
          ...
          for (i = 0; i < row.cell.length; i += 1) {
            ...
            if (
              i > 0 &&
              row.cell[i - 1].uid === row.cell[i].uid &&
              row.cell[i].type !== 1 &&
              row.cell[i].type !== 5 &&
              row.cell[i].rowpos === currentRowPos &&
              row.cell[i].colpos <= i
            ) {
      ...
      function addHeaders(tblparser) {
        ...
        for (i = 0; i < headStackLength; i += 1) {
          ...
          for (
            j = 0, cellLength = currRow.cell.length;
            j < cellLength;
            j += 1
          ) {
            ...
            if (
              (currCell.type === 1 || currCell.type === 7) &&
              !(j > 0 && currCell.uid === currRow.cell[j - 1].uid) &&
                !(
                  i > 0 &&
                  currCell.uid === tblparser.theadRowStack[i - 1].cell[j].uid
                )
            ) {
              // Imediate header
              ...
              if (i > 0) {
                ...
                for (
                  k = 0,
                    cellHeaderLength =
                      tblparser.theadRowStack[i - 1].cell[j].header.length;
                  k < cellHeaderLength;
                  k += 1
                ) {
                  currCell.headers.push(
                    tblparser.theadRowStack[i - 1].cell[j].header[k]
                  );
                  tblparser.theadRowStack[i - 1].cell[j].header[k].childs.push(
                    currCell
                  );
                }
        ...
        for (i = 0, rowLength = tblparser.row.length; i < rowLength; i += 1) {
          ...
          for (j = 0; j < currRow.cell.length; j += 1) {
            if (
              j === 0 ||
              (j > 0 && currRow.cell[j].uid !== currRow.cell[j - 1].uid)
            ) {
              currCell = currRow.cell[j];
              coldataheader = [];
              ...
              if (currCell.col && !currCell.col.dataheader) {
                ...
                if (currCol.headerLevel) {
                  for (
                    m = 0, headerLevelLength = currCol.headerLevel.length;
                    m < headerLevelLength;
                    m += 1
                  ) {
                    colheadersgroup = colheadersgroup.concat(
                      currCol.headerLevel[m]
                    );
                  }
                }
                if (currCol.header) {
                  for (
                    m = 0, headerLength = currCol.header.length;
                    m < headerLength;
                    m += 1
                  ) {
              ...
              if (currCell.type === 1) {
                ...
                for (
                  m = 0, ongoingRowHeaderLength = ongoingRowHeader.length;
                  m < ongoingRowHeaderLength;
                  m += 1
                ) {
                  if (
                    currCell.colpos ===
                    ongoingRowHeader[m].colpos + ongoingRowHeader[m].width
                  ) {
                    childLength = ongoingRowHeader[m].child.length;
                    if (
                      childLength === 0 ||
                      (childLength > 0 &&
                        ongoingRowHeader[m].child[childLength - 1].uid !==
                          currCell.uid)
                    ) {
              ...
              if (currCell.type === 2 || currCell.type === 3) {
                ...
                if (currCell.addcolheaders) {
                  for (
                    m = 0, addColHeadersLength = currCell.addcolheaders.length;
                    m < addColHeadersLength;
                    m += 1
                  ) {
                    coldataheader = coldataheader.concat(
                      currCell.addcolheaders[m]
                    );
                  }
                }
                ...
                if (currCell.addrowheaders) {
                  for (
                    m = 0, addRowHeadersLength = currCell.addrowheaders.length;
                    m < addRowHeadersLength;
                    m += 1
                  ) {
                    currrowheader = currrowheader.concat(
                      currCell.addrowheaders[m]
                    );
                  }
                }
      ...
      $obj.children().each(function() {
        ...
        if (nodeName === "caption") {
          ...
          if (
            theadRowStack.length !== 0 ||
            (groupZero.row && groupZero.row.length > 0)
          ) {
            $obj.trigger({
              type: warningEvent,
              pointer: this,
