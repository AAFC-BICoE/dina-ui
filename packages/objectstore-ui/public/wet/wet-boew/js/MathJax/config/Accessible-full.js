...
(function(j, b, d) {
  ...
  d.Augment({
    ...
    Startup: function() {
      ...
      this.linebreakSpan = MathJax.HTML.Element(
        "span",
        { className: "MathJax_LineBox" },
        [["span"]]
      );
    ...
    placeBox: function(v, u, s, p) {
      v.isMathJax = true;
      var w = d.parentNode(v),
        E = v.bbox,
        A = w.bbox;
      if (this.msiePlaceBoxBug) {
        this.addText(v, this.NBSP);
      }
      if (this.imgSpaceBug) {
        this.addText(v, this.imgSpace);
      }
      var z,
        G = 0;
      if (v.HH != null) {
        ...
        if (E) {
          z = Math.max(3, E.h + E.d);
        } else {
          z = v.offsetHeight / this.em;
        }
      }
      if (!v.noAdjust) {
        z += 1;
        z = Math.round(z * this.em) / this.em;
        if (this.msieInlineBlockAlignBug) {
          this.addElement(v, "img", {
            className: "MathJax_strut",
            border: 0,
            src: "about:blank",
            isMathJax: true,
            style: { width: 0, height: this.Em(z) }
          });
        } else {
          this.addElement(v, "span", {
            isMathJax: true,
            style: { display: "inline-block", width: 0, height: this.Em(z) }
          });
          if (d.chromeHeightBug) {
            z -= (v.lastChild.offsetHeight - Math.round(z * this.em)) / this.em;
          }
        }
      }
      if (E) {
        if (this.initialSkipBug) {
          if (E.lw < 0) {
            G = E.lw;
            d.createBlank(v, -G, true);
          }
          if (E.rw > E.w) {
            d.createBlank(v, E.rw - E.w + 0.1);
          }
        }
        if (!this.msieClipRectBug && !E.noclip && !p) {
          var C = 3 / this.em;
          var B = E.H == null ? E.h : E.H,
            n = E.D == null ? E.d : E.D;
          var F = z - B - C,
            q = z + n + C,
            o = -1000,
      ...
      v.style.top = this.Em(-s - z);
      v.style.left = this.Em(u + G);
      if (E && A) {
        if (E.H != null && (A.H == null || E.H + s > A.H)) {
          A.H = E.H + s;
        }
        if (E.D != null && (A.D == null || E.D - s > A.D)) {
          A.D = E.D - s;
        }
        if (E.h + s > A.h) {
          A.h = E.h + s;
        ...
        if (E.width != null && !E.isFixed) {
          if (A.width == null) {
            w.style.width = A.width = "100%";
            if (E.minWidth) {
              w.style.minWidth = A.minWidth = E.minWidth;
            }
          }
          v.style.width = E.width;
        }
        if (E.tw) {
          A.tw = E.tw;
        }
    ...
    handleChar: function(o, m, u, t, s) {
      ...
      if (u[2] || !this.msieAccentBug || s.length) {
        return s + r.c;
      }
