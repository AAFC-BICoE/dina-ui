  ...
  runTest('Input type="date" polyfill (date picker)', function() {
    var sandbox = sinon.sandbox.create(),
      ...
      beforeFactory = function(elm, label) {
        ...
        return function(done) {
          ...
          $formGroup = $("<div>" + label + "</div>").prependTo($body);

          $elm = $(elm).appendTo($formGroup);
    ...
    describe("initialization", function() {
      ...
      it("should have added links to the calendar", function() {
        ...
        expect($calendar.find(".cal-days a").length).to.equal(
          lastDay.getDate()
        );
      });
      ...
      it("should have stored a state object in the field element", function() {
        ...
        expect(state.minDate.toString()).to.equal(
          new Date(1800, 0, 1).toString()
        );
        expect(state.maxDate.toString()).to.equal(
          new Date(2100, 0, 1).toString()
        );
    ...
    describe("with a populated date", function() {
      before(
        beforeFactory(
          '<input type="date" id="appointment" value="2014-08-07"/>'
        )
      );
      after(defaultAfter);
    ...
    describe("toggle button", function() {
      var hiddenAltText =
        "Pick a date from a calendar for field: Appointment Date";
    ...
    describe("opening the calendar", function() {
      before(function(done) {
        ...
        beforeFactory(
          '<input type="date" id="test" min="2014-03-04" max="2014-03-18"/>',
      ...
      it("should have updated the aria-labelled-by to the toggle button", function() {
        expect($calendar.attr("aria-labelledby")).to.equal(
          "test-picker-toggle"
        );
      });
      ...
      it("should have positioned the date picker immediately under the control", function() {
        ...
        expect(Math.floor(calendarPosition.left)).to.equal(
          Math.floor(fieldPosition.left)
        );
        expect(Math.floor(calendarPosition.top)).to.equal(
          Math.floor(fieldPosition.top + $elm.outerHeight())
        );
      });
    });
    ...
    describe("opening the calendar when the associated field has a date that is inside the date range", function() {
      before(function(done) {
        ...
        beforeFactory(
          '<input type="date" id="appointment" min="2014-03-18" max="2015-03-18" value="2014-08-07"/>'
        )();
      });
      after(function() {
        $elm
      ...
      it("should have highlighted the selected date", function() {
        expect(
          $calendar.find(".cal-index-7 > a").attr("aria-selected")
        ).to.equal("true");
      });
    });
    ...
    describe("minimum date", function() {
      before(function(done) {
        ...
        beforeFactory(
          '<input type="date" id="appointment" min="2014-03-18"/>'
        )();
      });
      after(function() {
        $elm
      ...
      it("should have been added to the state object", function() {
        expect($elm.get(0).state.minDate.toString()).to.equal(
          new Date(2014, 2, 18).toString()
        );
      });

      it("should have been passed the minimum to the calendar plugin", function() {
        expect($calendar.get(0).firstChild.lib.minDate.toString()).to.equal(
          new Date(2014, 2, 18).toString()
        );
      });
    ...
    describe("maximum date", function() {
      before(function(done) {
        ...
        beforeFactory(
          '<input type="date" id="appointment" max="2014-03-18"/>'
        )();
      });
      after(function() {
        $elm
      ...
      it("should have been added to the state object", function() {
        expect($elm.get(0).state.maxDate.toString()).to.equal(
          new Date(2014, 2, 18).toString()
        );
      });

      it("should have been passed the minimum to the calendar plugin", function() {
        expect($calendar.get(0).firstChild.lib.maxDate.toString()).to.equal(
          new Date(2014, 2, 18).toString()
        );
      });
    ...
    describe("minimum and maximum dates in same month", function() {
      before(function(done) {
        ...
        beforeFactory(
          '<input type="date" id="appointment" min="2014-03-04" max="2014-03-18"/>'
        )();
      });
      after(function() {
        $elm
      ...
      it("should have been passed the minimum and maximum to the calendar plugin", function() {
        var settings = $calendar.get(0).firstChild.lib;
        expect(settings.minDate.toString()).to.equal(
          new Date(2014, 2, 4).toString()
        );
        expect(settings.maxDate.toString()).to.equal(
          new Date(2014, 2, 18).toString()
        );
      });
    ...
    describe("selecting a date", function() {
      before(function(done) {
        ...
        beforeFactory(
          '<input type="date" id="appointment" min="2014-03-01" max="2014-03-31"/>'
        )();
      });
      after(defaultAfter);
    ...
    describe("updating the min and max date of the field after the creation of the element", function() {
      before(function(done) {
        ...
        beforeFactory(
          '<input type="date" id="appointment" min="2014-03-08" max="2014-03-22"/>'
        )();
      });
      after(function() {
        $elm
      ...
      it("should have update the field's state", function() {
        ...
        expect(state.minDate.toString()).to.equal(
          new Date(2014, 2, 12).toString()
        );
        expect(state.maxDate.toString()).to.equal(
          new Date(2014, 2, 17).toString()
        );
      });
