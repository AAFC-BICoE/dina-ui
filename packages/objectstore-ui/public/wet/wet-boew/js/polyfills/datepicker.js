...
(function($, window, document, wb) {
  ...
  var componentName = "wb-date",
      ...
      if (elm) {
        ...
        if (!i18nText) {
          ...
          i18nText = {
            ...
            hide:
              i18n("date-hide").replace("\\'", "'") +
              space +
              space +
              i18n("esc-key").replace("\\'", "'"),
    ...
    createCalendar = function(elm) {
      ...
      $container = $(
        "<div id='" +
          containerName +
          "' class='picker-overlay' role='dialog' tabindex='-1' aria-hidden='true'></div>"
      );
    ...
    createToggleIcon = function(elm) {
      var fieldId = elm.id,
        ...
        objToggle =
          "<span class='input-group-btn'><a href='javascript:;' button id='" +
          fieldId +
          "-picker-toggle' class='btn btn-default picker-toggle' href='javascript:;' title=\"" +
          showFieldLabelText +
          "\"><span class='glyphicon glyphicon-calendar'></span><span class='wb-inv'>" +
          showFieldLabelText +
          "</span></a></span>";
    ...
    hide = function() {
      ...
      $container.removeClass("open").attr("aria-hidden", "true");
  ...
  $document.on("focusout focusin", "#" + containerName + " .wb-clndr", function(
    event
  ) {
  ...
  $document.on("click", ".picker-overlay .cal-days a", function(event) {
    ...
    if ((!which || which === 1) && !field.disabled && !field.readOnly) {
      field.value = $(event.currentTarget)
        .find("time")
        .attr("datetime");
  ...
  $document.on("click", ".picker-toggle", function(event) {
    ...
    if (!which || which === 1) {
      ...
      field = $(
        "#" + wb.jqEscape(pickerId.substring(0, pickerId.indexOf(toggleSuffix)))
      ).get(0);
