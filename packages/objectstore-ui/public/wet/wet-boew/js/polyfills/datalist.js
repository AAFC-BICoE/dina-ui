...
(function($, window, document, wb) {
  ...
  var componentName = "wb-datalist",
    ...
    populateOptions = function(input) {
      var $input = $(input),
        autolist =
          "<div class='wb-al-cnt'><ul role='listbox' id='wb-al-" +
          input.id +
          "' class='wb-al hide' aria-hidden='true' aria-live='polite'></ul>",
    ...
    showOptions = function(input, value) {
      ...
      if (value && value.length !== 0) {
        ...
        $options = $options.filter(function() {
          ...
          return (
            comparator.length === 0 ||
            value.toLowerCase().indexOf(comparator) === 0
          );
        });
      }
  ...
  $document.on(
    "timerpoke.wb " +
      ...
      " " +
      updateEvent +
      " keydown click vclick touchstart",
    selector,
    function(event) {
      var input = event.target,
  ...
  $document.on(
    "focusin txt-rsz.wb win-rsz-width.wb win-rsz-height.wb",
    function(event) {
          ...
          if (focusEvent) {
            autolistContainer =
              ...
              $(input)
                ...
                .get(0);
            if (
              autolistContainer.firstChild.className.indexOf("hide") === -1 &&
