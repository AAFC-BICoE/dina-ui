...
var fdSlider = (function() {
  ...
  var parseJSON = function(str) {
    ...
    try {
      ...
      if (typeof JSON === "object" && typeof JSON.parse === "function") {
        ...
        /mousewheelenabled|fullaria|describedby|norangebar|html5animation|varsetrules/.test(
          str.toLowerCase()
        )
      ) {
        var f = Function(
  ...
  var affectJSON = function(json) {
    ...
    for (var key in json) {
      ...
      switch (key.toLowerCase()) {
        ...
        case "html5animation":
          html5Animation =
            String(value).search(/^(jump|tween|timed)$/i) != -1
  ...
  var createSlider = function(options) {
    ...
    if (options.inp.tagName.toLowerCase() == "select") {
      ...
      options.min =
        options.min && String(options.min).search(fpRegExp) != -1
          ? +options.min
          : 0;
      options.max =
        options.max && String(options.max).search(fpRegExp) != -1
          ? +options.max
          : 100;
      options.step =
        options.step && String(options.step).search(stepRegExp) != -1
          ? options.step
          : 1;
      options.precision =
        options.precision && String(options.precision).search(/^[0-9]+$/) != -1
          ? options.precision
          : String(options.step).search(/\.([0-9]+)$/) != -1
          ? String(options.step).match(/\.([0-9]+)$/)[1].length
          : 0;
      options.scale = options.scale || false;
      options.forceValue =
        "forceValue" in options ? !!options.forceValue : false;
      options.userSnap = "userSnap" in options ? !!options.userSnap : false;
    }
    options.ariaFormat =
      "ariaFormat" in options && typeof options.ariaFormat == "function"
        ? options.ariaFormat
        : false;
    options.maxStep =
      options.maxStep && String(options.maxStep).search(stepRegExp) != -1
        ? +options.maxStep
        : +options.step * 2;
  ...
  var init = function() {
    ...
    for (var i = 0, inp; (inp = inputs[i]); i++) {
      if (
        ...
        getAttribute(inp, "type") &&
          getAttribute(inp, "type").toLowerCase() == "range" &&
        ((getAttribute(inp, "min") &&
          getAttribute(inp, "min").search(fpRegExp) != -1) ||
          (getAttribute(inp, "max") &&
            getAttribute(inp, "max").search(fpRegExp) != -1) ||
          (getAttribute(inp, "step") &&
            getAttribute(inp, "step").search(
              /^(any|([0-9]+(\.[0-9]+){0,1}))$/i
            ) != -1))
      ) {
        ...
        options = {
          ...
          vertical: getAttribute(inp, "data-fd-slider-vertical")
            ? true
            : inp.offsetHeight > inp.offsetWidth,
  ...
  var getInputAttributes = function(inp) {
    return {
      ...
      step: +(getAttribute(inp, "step").search(stepRegExp) != -1
        ? inp.getAttribute("step")
        : 1)
    };
  };
  ...
  function fdRange(options) {
    var inp = options.inp,
      ...
      resetDef =
        tagName == "select"
          ? inp.selectedIndex
          : inp.defaultValue || defaultVal,
      forceValue = html5Shim || !!options.forceValue,
      inpHeight =
        html5Shim && vertical && "inpHeight" in options
          ? options.inpHeight
          : false,
      ariaFormat =
        !html5Shim && options.ariaFormat ? options.ariaFormat : false,
      userSnap =
        !html5Shim && !(tagName == "select") && "userSnap" in options
          ? !!options.userSnap
          : false,
      userInput = false,
      timer = null,
    ...
    function redraw() {
      ...
      try {
        ...
        rMinPx = Math.max(
          scale
            ? percentToPixels(valueToPercent(rMin))
            : Math.abs((rMin - min) / step) * stepPx,
          0
        );
        rMaxPx = Math.min(
          scale
            ? percentToPixels(valueToPercent(rMax))
            : Math.abs((rMax - min) / step) * stepPx,
    ...
    function onMouseDown(e) {
      ...
      if (targ.className.search("fd-slider-handle") != -1) {
        ...
        handlePos =
          parseInt(vertical ? handle.offsetTop : handle.offsetLeft) || 0;
        ...
        addClass(
          document.body,
          "fd-slider-drag-" + (vertical ? "vertical" : "horizontal")
        );
        ...
        posx -= vertical
          ? y + Math.round(handle.offsetHeight / 2)
          : x + Math.round(handle.offsetWidth / 2);
    ...
    function trackMouse(e) {
      ...
      pixelsToValue(
        snapToPxValue(
          handlePos + (vertical ? e.clientY - mousePos : e.clientX - mousePos)
        )
      );
    ...
    function getWorkingValueFromInput() {
      return getValidValue(
        tagName == "input" ? parseFloat(inp.value) : inp.selectedIndex
      );
    }
    ...
    function getValidValue(value) {
      return isNaN(value) || value === "" || typeof value == "undefined"
        ? defaultVal
        : Math.min(Math.max(value, Math.min(rMin, rMax)), Math.max(rMin, rMax));
    }

    // Calculates value according to pixel position of slider handle
    function pixelsToValue(px) {
      var val = getValidValue(
        scale
          ? percentToValue(pixelsToPercent(px))
          : vertical
          ? max - Math.round(px / stepPx) * step
          : min + Math.round(px / stepPx) * step
      );
    ...
    function valueToPixels(val, updateInputValue) {
      ...
      if (
        (typeof val === "undefined" || isNaN(val) || val === "") &&
        tagName == "input" &&
        !forceValue
      ) {
        value = defaultVal;
      ...
      handle.style[vertical ? "top" : "left"] =
        (scale
          ? percentToPixels(valueToPercent(value))
          : vertical
    ...
    function percentToPixels(percent) {
      return (
        ((outerWrapper[vertical ? "offsetHeight" : "offsetWidth"] -
          handle[vertical ? "offsetHeight" : "offsetWidth"]) /
          100) *
        percent
      );
    }
    ...
    function pixelsToPercent(pixels) {
      return (
        pixels /
        ((outerWrapper[vertical ? "offsetHeight" : "offsetWidth"] -
          outerWrapper[handle ? "offsetHeight" : "offsetWidth"]) /
          100)
      );
    }
    ...
    function setInputValue(val) {
      ...
      if (tagName == "select") {
        ...
        if (val !== "" && !userInput) {
          val = (min + Math.round((+val - min) / step) * step).toFixed(
            precision
          );
        }
        if (inp.value === val) {
    ...
    function checkInputValue(value) {
      return !(
        isNaN(value) ||
        value === "" ||
        value < Math.min(rMin, rMax) ||
        value > Math.max(rMin, rMax)
      );
    }
    ...
    function redrawRange() {
      ...
      if (vertical) {
        rangeBar.style["height"] =
          Math.max(1, bar.offsetHeight - handle.offsetTop) + "px";
      } else {
    ...
    function findLabel() {
      ...
      for (var i = 0, lbl; (lbl = labelList[i]); i++) {
        ...
        if (
          (lbl["htmlFor"] && lbl["htmlFor"] == inp.id) ||
    ...
    function onReset(e) {
      ...
      checkValue(
        tagName == "select" ? inp.options[inp.selectedIndex].value : inp.value
      );
      redraw();
      updateAriaValues();
    }
    ...
    (function() {
      ...
      if (html5Shim) {
        ...
        if (useDOMAttrModEvt) {
          addEvent(
            inp,
            typeof inp.onpropertychange == "object"
              ? "propertychange"
              : "DOMAttrModified",
            rescanAttrs
          );
      ...
      outerWrapper.className =
        "fd-slider" + (vertical ? "-vertical " : " ") + classNames;
      ...
      innerWrapper.className =
        "fd-slider-wrapper" + (!html5Shim ? " fd-slider-no-value" : "");
      ...
      handle.setAttribute(
        "aria-valuemin",
        tagName == "select" ? inp.options[0].value : min
      );
      handle.setAttribute(
        "aria-valuemax",
        tagName == "select" ? inp.options[inp.options.length - 1].value : max
      );
      ...
      if (
        inp.getAttribute("disabled") == true ||
        inp.getAttribute("disabled") == "disabled"
      ) {
        disableSlider(true);
      } else {
        enableSlider(true);
    ...
    return {
      onResize: function(e) {
        if (
          outerWrapper.offsetHeight != sliderH ||
          outerWrapper.offsetWidth != sliderW
        ) {
          redraw();
        }
      },
  ...
  (function() {
    var scriptFiles = document.getElementsByTagName("script"),
      json = parseJSON(
        String(scriptFiles[scriptFiles.length - 1].innerHTML)
          .replace(/[\n\r\s\t]+/g, " ")
          .replace(/^\s+/, "")
          .replace(/\s+$/, "")
      );
