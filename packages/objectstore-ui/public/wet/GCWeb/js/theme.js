...
(function($, wb, document) {
  ...
  var $document = wb.doc,
    ...
    tblflrAct = function(event, data) {
      ...
      $datatable
        .column(column)
        .search(data.value, regex, smart, caseinsen)
        .draw();
    },
    ...
    tblToCSV = function(source, fname) {
      var $table = $(source),
        ...
        j,
        columns_len = rows[0].cells.length,
        $datatable;
    ...
    withInput = function(event, data) {
      ...
      executePreRenderAction(
        elm.id,
        cValue,
        settingQuery.actions,
        dontTriggerWET
      );
    },
    ...
    patchFixArray = function(patchArray, val, basePointer) {
      ...
      for (i = 0; i !== i_len; i += 1) {
        ...
        if (i_cache.value) {
          patch_cache.value = replaceMappingKeys(
            i_cache.value,
            val,
            basePointer
          );
  ...
  $document.on("do." + actionEvent, function(event) {
    ...
    if (
      (elm === event.target || event.currentTarget === event.target) &&
      elm.className.indexOf(componentName) === -1
    ) {
      if (!$.isArray(actions)) {
        actions = [actions];
      }
...
(function($, window, wb) {
  ...
  var componentName = "wb-data-json",
    ...
    jsonFetched = function(event) {
      ...
      if (showEmpty || typeOfContent !== "undefined") {
        ...
        if (!jsonType) {
          ...
          jsonType === "prop" &&
          attrname &&
          allowPropNames.test(attrname)
        ) {
          $elm.prop(attrname, content);
        } else if (
          jsonType === "attr" &&
          attrname &&
          allowAttrNames.test(attrname)
        ) {
          $elm.attr(attrname, content);
        } else if (
          typeof $elm[jsonType] === "function" &&
          allowJsonTypes.indexOf(jsonType) !== -1
        ) {
    ...
    applyTemplate = function(elm, settings, content) {
      var mapping = settings.mapping || [{}],
        ...
        elmAppendTo = elm,
        dataTable,
        template = settings.source
          ? document.querySelector(settings.source)
      ...
      if (
        elm.tagName === "TABLE" &&
        mapping &&
        elmClass.indexOf("wb-tables-inited") !== -1 &&
        ...
        for (i = 0; i < i_len; i += 1) {
          ...
          if (filterPassJSON(i_cache, filterTrueness, filterFaslseness)) {
            ...
            for (j = 0; j < mapping_len; j += 1) {
              cached_value.push(
                jsonpointer.get(content, basePntr + mapping[j])
              );
            }
        ...
        if (filterPassJSON(i_cache, filterTrueness, filterFaslseness)) {
          ...
          for (j = 0; j < mapping_len || j === 0; j += 1) {
            ...
            if (j_cache.placeholder) {
              ...
              cached_value = cached_textContent.replace(
                j_cache.placeholder,
                cached_value
              );
            }
    ...
    filterPassJSON = function(obj, trueness, falseness) {
      ...
      if (trueness_len || falseness_len) {
        for (i = 0; i < trueness_len; i += 1) {
          i_cache = trueness[i];
          isEqual = _equalsJSON(
            jsonpointer.get(obj, i_cache.path),
            i_cache.value
          );
  ...
  $document.on(
    "timerpoke.wb " + initEvent + " " + updateEvent + " json-fetched.wb",
    selector,
    function(event) {
...
(function($, window, wb) {
  ...
  var componentName = "wb-doaction",
    selector =
      "a[data-" + componentName + "],button[data-" + componentName + "]",
...
(function($, document, wb) {
  ...
  var componentName = "wb-fieldflow",
    ...
    fieldflowActionsEvents = [
      [
        ...
        "." +
        actionEvent,
      ["ajax", "toggle", "redir", "addClass", "removeClass"].join(
        "." + submitEvent + " "
      ) +
        "." +
        submitEvent,
      ["tblfilter", componentName].join("." + drawEvent + " ") +
        "." +
        drawEvent,
      ["select", "checkbox", "radio"].join("." + createCtrlEvent + " ") +
        "." +
        createCtrlEvent
    ].join(" "),
    /**
    ...
    init = function(event) {
      ...
      if (elm) {
        ...
        if (config.noForm) {
          stdOut =
            "<div class='mrgn-tp-md'><div id='" + bodyID + "'></div></div>";
          ...
          stdOut =
            ...
            i18n.btn +
            '" class="btn btn-primary mrgn-bttm-md" /> </form></div>';
        }
        $elm.addClass("hidden");
        stdOut = $(stdOut);
        $elm.after(stdOut);
      ...
      if (event.namespace === drawEvent) {
        ...
        if (filterSequence.length) {
          ...
          filterItm = {
            ...
            srctype: "tblfilter",
            source: sourceSelector,
            renderas: firstFilterSeq.renderas
              ? firstFilterSeq.renderas
              : renderas,
    ...
    ctrlSelect = function(event, data) {
      ...
      if (isReq && useReqLabel) {
        ...
        labelSuffix +=
          " <strong class='required'>(" + i18n.required + ")</strong>";
      }
      ...
      selectOut =
        ...
        basenameInput +
        autoID +
        "' class='full-width form-control mrgn-bttm-md " +
        crtlSelectClass +
        "' data-" +
        originData +
        "='" +
    ...
    ctrlChkbxRad = function(event, data) {
      ...
      if (!lblselector) {
        ...
        $out
          .append(fieldsetPrefix + $tmpLabel.html() + fieldsetSuffix)
      ...
      for (i = 0, i_len = items.length; i !== i_len; i += 1) {
        ...
        if (!cur_itm.group) {
          radCheckOut += buildCheckboxRadio(
            cur_itm,
            fieldName,
            typeRadCheck,
            isInline,
    ...
    buildDataAttribute = function(data) {
      ...
      out +=
        " data-" + componentName + "='" + JSON.stringify(dataFieldflow) + "'";
  ...
  $document.on(resetActionEvent, selector + ", ." + subComponentName, function(
    event
  ) {
    var elm = event.target,
      $elm,
  ...
  $document.on("change", selectorForm + " " + crtlSelectSelector, function(
    event
  ) {
    ...
    if (i_len) {
      for (i = i_len; i !== 0; i -= 1) {
        ...
        if (itm) {
          ...
          $("#" + itm.getAttribute(sourceDataAttr))
            .trigger(resetActionEvent)
            .trigger(cleanEvent);
  ...
  $document.on("submit", selectorForm + " form", function(event) {
    var elm = event.currentTarget,
      ...
      $hdnField,
      i,
      i_len = wbFieldFlowRegistered ? wbFieldFlowRegistered.length : 0,
      $wbFieldFlow,
      fieldOrigin,
    ...
    for (i = 0; i !== i_len; i += 1) {
      ...
      m_cache.$elm.trigger(
        m_cache.data.action + "." + submitedEvent,
        m_cache.data
      );
    }
    if (preventSubmit) {
...
(function($, window, wb) {
  ...
  var componentName = "wb-jsonmanager",
      ...
      ops: [
        {
          ...
          fn: function(obj, key, tree) {
            ...
            jsonpatch.apply(tree, [{ op: "add", path: this.set, value: len }]);
          }
        },
        {
          name: "wb-first",
            ...
            jsonpatch.apply(tree, [
              {
                op: "replace",
                path: this.path,
                value: prefix + val.toLocaleString(loc) + suffix
              }
            ]);
          }
        },
              ...
              jsonpatch.apply(tree, [
                {
                  op: "replace",
                  path: this.path,
                  value: wb.date.toDateISO(obj[key])
                }
              ]);
            } else {
              jsonpatch.apply(tree, [
              ...
              jsonpatch.apply(tree, [
                {
                  op: "replace",
                  path: this.path,
                  value: wb.date.toDateISO(obj[key], true)
                }
              ]);
            } else {
              jsonpatch.apply(tree, [
    ...
    debugPrintOut = function($elm, name, json, patches) {
      $elm.after(
        '<p lang="en"><strong>JSON Manager Debug</strong> (' +
          name +
          ')</p><ul lang="en"><li>JSON: <pre><code>' +
          JSON.stringify(json) +
          "</code></pre></li><li>Patches: <pre><code>" +
    ...
    getPatchesToFilter = function(
      JSONsource,
      filterPath,
      filterTrueness,
      filterFaslseness
    ) {
      var filterObj, i, i_len;
      ...
      if ($.isArray(filterObj)) {
        ...
        for (i = i_len; i !== -1; i -= 1) {
          if (!filterPassJSON(filterObj[i], filterTrueness, filterFaslseness)) {
            jsonpatch.apply(JSONsource, [
              { op: "remove", path: filterPath + "/" + i }
            ]);
          }
        }
      }
  ...
  $document.on("json-fetched.wb", selector, function(event) {
    ...
    if (elm === event.currentTarget) {
      ...
      // Apply a filtering
      if (filterPath) {
        JSONresponse = getPatchesToFilter(
          JSONresponse,
          filterPath,
          filterTrueness,
          filterFaslseness
  ...
  // Fieldflow "op" action
  $document.on("op.action.wb-fieldflow", ".wb-fieldflow", function(
    event,
    data
  ) {
    if (!data.op) {
  ...
  // Fieldflow "op" submit
  $document.on("op.submit.wb-fieldflow", ".wb-fieldflow", function(
    event,
    data
  ) {
    // Get the hbs Plugin
...
(function($, wb) {
  ...
  function OpenMenu(elm) {
    ...
    var menuOpen = parentMenu.querySelector(
  ...
  function CloseMenu(elm, force) {
    ...
    if (!force) {
      ...
      var currentFocusIsOn = elm.nextElementSibling.querySelector(
        "[role=menuitem]:focus"
      );
      var siblingHasFocus = elm.parentElement.parentElement.querySelector(
        "[role=menuitem]:focus"
      );
  ...
  $document.on(
    "mouseenter focusin",
    selector + " [aria-haspopup] + [role=menu]",
    function(event) {
  ...
  $document.on("click", selector + " [aria-haspopup]", function(event) {
    ...
    if (isMobileMode || elm.nodeName === "BUTTON") {
      ...
      if (elm.getAttribute("aria-expanded") === "true") {
        ...
        elmToGiveFocus = elm.nextElementSibling.querySelector(
          "[role=menuitem]"
        );
  ...
  function setMnu3LevelOrientationExpandState(isVertical, isExpanded) {
    var mnu3Level = document.querySelectorAll(
        "[role=menu] [role=menu] [role=menuitem][aria-haspopup=true]"
      ),
      i,
      i_len = mnu3Level.length,
    ...
    for (i = 0; i < i_len; i++) {
      ...
      expandStateItem = mnu3Level[i].nextElementSibling.querySelector(
        "[role=menuitem]:focus"
      )
        ? "true"
        : expandState;
      ...
      mnu3Level[i].parentElement.previousElementSibling.setAttribute(
        "aria-orientation",
        orientation
      );
    }
  ...
  $document.on(
    "keydown",
    selector + " button, " + selector + " [role=menuitem]",
    function(event) {
      var elm = event.currentTarget,
        key = keycode(event.charCode || event.keyCode);

      // Get the menu item that has the focus.
      var currentFocusIsOn =
          document.querySelector("[role=menuitem]:focus") || elm,
      ...
      if (currentFocusIsOn.nextElementSibling) {
        firstChildPopup = currentFocusIsOn.nextElementSibling.querySelector(
          "[role='menuitem']"
        );
      }
      ...
      if (parent.nextElementSibling) {
        nextSiblingMenuItem = parent.nextElementSibling.querySelector(
          "[role=menuitem]"
        );
        ...
        if (!nextSiblingMenuItem) {
          nextSiblingMenuItem = parent.nextElementSibling.nextElementSibling.querySelector(
            "[role=menuitem]"
          );
        }
      } else {
        ...
        if (
          !isMobileMode &&
          currentFocusIsOn.dataset.keepExpanded &&
          firstChildPopup
        ) {
          ...
          !isMobileMode &&
          grandParent.previousElementSibling.dataset.keepExpanded
        ) {
          ...
          nextSiblingMenuItem = grandParent.parentElement.parentElement.querySelector(
            "[role=menuitem]"
          );
        } else {
      ...
      if (parent.previousElementSibling) {
        previousSiblingMenuItem = parent.previousElementSibling.querySelector(
          "[role=menuitem]"
        );

        // Check if we have hit a separator. A separator is not the first items
        if (!previousSiblingMenuItem) {
          previousSiblingMenuItem = parent.previousElementSibling.previousElementSibling.querySelector(
            "[role=menuitem]"
          );
        }
      } else {
        ...
        if (
          !isMobileMode &&
          grandParent.lastElementChild.querySelector("[role=menuitem]").dataset
            .keepExpanded
        ) {
          // The last item is persistant open, get it's last children
          previousSiblingMenuItem = grandParent.lastElementChild
            .querySelector("[role=menuitem]")
            .nextElementSibling.lastElementChild.querySelector(
              "[role=menuitem]"
            );
        } else if (
          !isMobileMode &&
          ...
          previousSiblingMenuItem = currentFocusIsOn.nextElementSibling.lastElementChild.querySelector(
            "[role=menuitem]"
          );
        } else {
          // Get the last item of the current menu
          previousSiblingMenuItem = grandParent.lastElementChild.querySelector(
            "[role=menuitem]"
          );
        }
      }
      ...
      while (iteratedItem.nextElementSibling) {
        ...
        if (iteratedItem.getAttribute("role") === "separator") {
          if (
            iteratedItem.hasAttribute("aria-orientation") &&
            iteratedItem.getAttribute("aria-orientation") === "vertical"
          ) {
            isNextSeparatorOrientationVertical = true;
          } else {
            isNextSeparatorOrientationVertical = false;
          }
          nextSeparatorMenuItem = iteratedItem.nextElementSibling.querySelector(
            "[role=menuitem]"
          );
          break;
        }
      ...
      var isPreviousSeparatorOrientationVertical, previousSeparatorMenuItem;
      iteratedItem = parent;
      ...
      if (previousSeparatorMenuItem) {
        previousSeparatorMenuItem = previousSeparatorMenuItem.querySelector(
          "[role=menuitem]"
        );
      }
	...
	*/
      ...
      if (key === "down" && nextSiblingMenuItem) {
        ...
        (!isCurrentButtonMenu && key === "right" && firstChildPopup) ||
        (key === "enter" && firstChildPopup)
      ) {
        ...
        (!isCurrentButtonMenu && key === "left") ||
        (!isCurrentButtonMenu && key === "esc")
      ) {
      ...
      if (elmToGiveFocus) {
        ...
        if (isMobileMode || isCurrentButtonMenu) {
          var popup =
            elmToGiveFocus.parentElement.parentElement.previousElementSibling;
  ...
  $document.on("ajax-fetched.wb ajax-failed.wb", selectorAjaxed, function(
    event
  ) {
    var elm = event.target;
...
(function($, document, wb) {
        ...
        $(this).trigger({
          ...
          fetch: {
            url:
              ...
              "//clients1.google.com/complete/search?client=partner&sugexp=gsnos%2Cn%3D13&gs_rn=25&gs_ri=partner&partnerid=" +
              window.encodeURIComponent(
                "008724028898028201144:knjjdikrhq0+lang:" + wb.lang
              ) +
              "&types=t&ds=cse&cp=3&gs_id=b&hl=" +
              wb.lang +
              "&q=" +
              encodeURI(query),
            dataType: "jsonp",
  ...
  $document.on("ajax-fetched.wb", searchSelector, function(event) {
    ...
    for (indIssue = 0; indIssue < lenSuggestions; indIssue += 1) {
      ...
      options +=
        '<option label="' + issue[0] + '" value="' + issue[0] + '"></option>';
    }
