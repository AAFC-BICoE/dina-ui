...
import { SeqBatch, PcrBatchItem } from "../../../types/seqdb-api";
...
export function SangerPcrBatchSelectionStep({
  ...
  async function fetchSampledIds() {
    await apiClient
      .get<PcrBatchItem[]>("/seqdb-api/seq-batch-item", {
        filter: filterBy([], {
          extraFilters: [
            {
      ...
      .then((response) => {
        const seqBatchItems: PersistedResource<PcrBatchItem>[] =
          response?.data?.filter(
            (item) => item?.materialSample?.id !== undefined
          );
        const materialSampleIds: string[] =
          seqBatchItems.map((item) => item?.materialSample?.id as string) ?? [];

        setPreviouslySelectedResources(seqBatchItems);
