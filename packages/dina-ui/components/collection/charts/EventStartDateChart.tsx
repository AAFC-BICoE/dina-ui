...
/**
 * EventStartDateChart component.
 *
 * Renders a histogram displaying the distribution of collecting event start dates,
...
export default function EventStartDateChart({
  ...
  const addClickToQuery = (params: { name: string }) => {
    ...
    function createDateRule(selectedPreset: string, dateStr: string): any {
      ...
      // For hour and day intervals, we can filter for an exact match on the collecting event date field.
  ...
  const buildQuery = () => {
    ...
    // Ensure the query has a nested filter for collecting-event
    ...
    if (dateRange.start || dateRange.end) {
      ...
      if (dateRange.end) {
        rangeFilter.range["included.attributes.startEventDateTime"].lte =
          dateRange.end;
      }
    ...
    // Add nested filter to query if not already present
    if (!query.bool.must) {
      query.bool.must = [];
    }
          ...
          aggs: {
            filtered_by_type: {
              ...
              aggs: {
                date_histogram_agg: {
                  date_histogram: {
                    field: "included.attributes.startEventDateTime",
