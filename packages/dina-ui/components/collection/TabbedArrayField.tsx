...
/**
 * Renders an array of objects as tabs, one element per tab. Use when the
 * elements are large enough that showing them all at once is unwieldy;
 * otherwise see InlineArrayField.
 */
export function TabbedArrayField<T>({
  ...
  return (
    <ArrayFieldContainer<T>
      ...
      onElementAdded={(index) => setActiveTabIdx(index)}
      // Stay on the current tab number, or reduce if removing the last element:
      onElementRemoved={(_index, remaining) =>
        setActiveTabIdx((current) => _.clamp(current, 0, remaining - 1))
      }
    >
      {({
        elements,
