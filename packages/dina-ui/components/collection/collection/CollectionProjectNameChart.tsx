...
export default function CollectionProjectNameChart({
  query
  ...
  const fetchData = async () => {
    ...
    if (response.data.aggregations) {
      const buckets =
        response.data.aggregations["nested#projects_nested"]?.[
          "filter#only_projects"
  ...
  return chartData.length != 0 ? (
    <div>
      <strong>Sample Count By Project</strong>
